...
camera_position_s camera_postion{};
...
namespace utilities {
	...
	public:
		...
		auto InScreen(FVector2d screen_position) -> bool {
			...
			if (screen_position.x > 0 && screen_position.x < globals::width && screen_position.y > 0 && globals::height < 1080)
				return true;
			else
				return false;
		...
		static auto GetBoneLocation(uintptr_t skeletal_mesh, int bone_index) -> FVector {
			...
			uintptr_t bone_array = read<uintptr_t>(skeletal_mesh + offset::BoneArray);
			...
			component_to_world = read<ftransform>(skeletal_mesh + offset::ComponentToWorld);
		...
		static auto GetBoneLocation3(uintptr_t skeletal_mesh, int bone_index) -> Vector3 {
			...
			if (bone_array == NULL) bone_array = read<uintptr_t>(skeletal_mesh + offset::BoneArray + 0x10);
			ftransform bone = read<ftransform>(bone_array + (bone_index * 0x60));
			ftransform component_to_world = read<ftransform>(skeletal_mesh + offset::ComponentToWorld);
			D3DMATRIX matrix = MatrixMultiplication(bone.ToMatrixWithScale(), component_to_world.ToMatrixWithScale());
		...
		{
			uintptr_t cachedgworld = read<uintptr_t>(imagebase + offset::UWorld);
			uintptr_t cachedgameinstance = read<uintptr_t>(cachedgworld + offset::OwningGameInstance);
			uintptr_t cachedlocalplayers = read<uintptr_t>(cachedgameinstance + offset::LocalPlayers);
			uintptr_t cachedlocalplayer = read<uintptr_t>(cachedlocalplayers);
			uintptr_t cachedplayercontroller = read<uintptr_t>(cachedlocalplayer + offset::PlayerController);

			cdecrypt camera;

			auto locationp = read<uintptr_t>(cachedgworld + 0x128);
			auto rotationp = read<uintptr_t>(cachedgworld + 0x138);
			...
			camera.fov = read<float>((uintptr_t)cachedplayercontroller + 0x3AC) * 90.f;
		...
		/*static auto WorldToScreen(FVector worldlocation) -> FVector2d {
			...
			float xcenter = (globals::width / 2.0f);
			float ycenter = (globals::height / 2.0f);


			ScreenLoction.x = xcenter + vTransformed.x * (xcenter / tanf(camera::fov * (float)M_PI / 360.f)) / vTransformed.z;
			ScreenLoction.y = ycenter - vTransformed.y * (ycenter / tanf(camera::fov * (float)M_PI / 360.f)) / vTransformed.z;

			return ScreenLoction;
		...
		Vector3 GetBoneWithRotation(DWORD_PTR mesh, int id)
		{
			...
			ftransform ComponentToWorld = read<ftransform>(mesh + offset::ComponentToWorld);
		...
		Vector3 ProjectWorldToScreen(Vector3 WorldLocation)
		{
			...
			auto chain69 = read<uintptr_t>(offset::LocalPlayers + 0xA8);
			uint64_t chain699 = read<uintptr_t>(chain69 + 8);

			Camera.x = read<float>(chain699 + 0x7F8);
			//printf("Camera.x :0x%llX\n", Camera.x);
			Camera.y = read<float>(offset::RootComponent + 0x12C);
			...
			uint64_t chain = read<uint64_t>(offset::LocalPlayers + 0x70);
